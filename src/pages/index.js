...
const ViewData = () => {
    ...
    const scanTable = () => {
        ...
        const postData = async ()=>{
            const response = await fetch("/api/viewdatadb", {
                method: "GET",
            });
        ...
        postData().then((data)=>{
            ...
            console.log("ViewedData", data.Items);
        })

    };

    //   deleting an item from the table
    const deleteItem =  (primaryKeyValue, sortKeyValue) => {
        ...
        const postData = async ()=>{
            const response = await fetch("/api/deletedatadb", {
                method: "DELETE",
